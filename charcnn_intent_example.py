...
# onehot과 wordvec 사용시의 learning rate가 다름.
learning_rate = 1e-3
...
def train_vector_model(data_list):
    ...
    model.train(morphs, total_examples=len(str_buf), epochs=5)
    return model
...
def extract_features(text):
	...
	for pos in raw_pos:
		...
		# 명사와 동사면 충분할듯?
		if pos[1] in ['NNG', 'NNP', 'VV']:
			res.append(pos[0])
		if pos[1].startswith('VV'):
			res.append(pos[0])
	return res
...
if __name__=='__main__':
	...
	while True:
		...
		if not text:
			break
		predict(np.array(inference_embed(text)).flatten())
